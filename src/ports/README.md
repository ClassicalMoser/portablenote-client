...
- `VaultPort` (`vault.ts`) — vault lifecycle, block/edge commands, `listBlocks` query.
...
- `VaultQueryPort` additions as Rust read commands land: `listEdges`, `getDocuments`, orphan query. These block the graph and document views.
